...
class AppState {
    constructor() {
        ...
        this.goals = [];
        this.contentItems = [
            { id: 1, title: 'Physics: Mechanics', type: 'video', youtube: 'https://www.youtube.com/results?search_query=physics+mechanics+lecture' },
            { id: 2, title: 'Chemistry: Organic', type: 'notes', youtube: 'https://www.youtube.com/results?search_query=organic+chemistry+lecture' },
            { id: 3, title: 'Math: Calculus I', type: 'video', youtube: 'https://www.youtube.com/results?search_query=calculus+1+lecture' },
            { id: 4, title: 'Biology: Genetics', type: 'quiz', youtube: 'https://www.youtube.com/results?search_query=genetics+lecture' },
            { id: 5, title: 'Physics: Thermodynamics', type: 'notes', youtube: 'https://www.youtube.com/results?search_query=thermodynamics+lecture' },
            { id: 6, title: 'English: Grammar', type: 'practice', youtube: 'https://www.youtube.com/results?search_query=english+grammar+lessons' }
        ];
...
// Enabled: clear stored user data when the page is refreshed/reloaded.
...
function updateDashboard() {
    ...
    // update left-side goals card if present
    try { renderLeftGoalsCard(); } catch (e) {}
}
    ...
    grid.innerHTML = state.contentItems.map(item => {
        ...
        <div class="content-item" data-id="${item.id}">
            ...
            <div class="content-actions">
                ...
                <button class="icon-btn liked" title="Like" onclick="toggleLikeContent(${item.id}, this)">${liked ? '❤️ Liked' : '♡ Like'}</button>
                ${item.youtube ? `<button class="btn-secondary" onclick="window.open('${item.youtube}','_blank')">▶ Watch</button>` : ''}
            </div>
        </div>
        `;
    }).join('');
...
// Render a compact left-side goals card on the dashboard
function renderLeftGoalsCard() {
    const el = document.getElementById('leftGoalsList');
    if (!el) return;
    if (!state.goals || state.goals.length === 0) {
        el.innerHTML = '<div class="empty-state">No goals yet. Add one from Weekly Goals.</div>';
        return;
    }
    el.innerHTML = state.goals.map(g => `
        <div class="goal-item" data-id="${g.id}">
            <input type="checkbox" class="goal-checkbox" ${g.completed ? 'checked' : ''} onchange="state.toggleGoal(${g.id}); renderLeftGoalsCard(); updateDashboard(); renderGoals();">
...
// Render goals inline on dashboard (scroll-to view)
function renderDashboardGoals() {
    const container = document.getElementById('dashboardGoalsList');
    if (!container) return;
    if (!state.goals || state.goals.length === 0) {
        container.innerHTML = '<div class="empty-state">No weekly goals — add one below.</div>';
        return;
    }
    container.innerHTML = state.goals.map(g => `
        <div class="goal-item">
            <input type="checkbox" class="goal-checkbox" ${g.completed ? 'checked' : ''} onchange="state.toggleGoal(${g.id}); updateDashboard(); renderGoals(); renderLeftGoalsCard(); renderDashboardGoals(); renderMilestones(); renderDashboardMilestones();">
            <span class="goal-text ${g.completed ? 'completed' : ''}">${g.text}</span>
        </div>
    `).join('');
}
...
if(loginForm) loginForm.addEventListener('submit', (e) => {
    ...
    // detect if a different user previously used this browser; if so, reset stored data
    ...
    // persist the last-used email for future detection
...
// Left-side Manage Goals button (on dashboard) opens the goals page
const leftAddBtn = document.getElementById('leftAddGoalBtn');
if (leftAddBtn) leftAddBtn.addEventListener('click', () => { showPage('goalsPage'); renderGoals(); renderWeekCalendar(); });
...
function showContentModal(content) {
    ...
    if(bodyEl) bodyEl.innerHTML = `
        ...
        ${content.youtube ? `<div style="margin-top:12px"><button class="btn-primary" onclick="window.open('${content.youtube}','_blank')">Watch on YouTube</button></div>` : ''}
    `;
...
if (state.isLoggedIn) {
    ...
    renderSavedLikedCards();
    try { renderLeftGoalsCard(); } catch(e) {}
    try { renderDashboardGoals(); } catch(e) {}
    try { renderDashboardMilestones(); } catch(e) {}
...
// Render milestones inline on dashboard
function renderDashboardMilestones() {
    const container = document.getElementById('dashboardMilestonesList');
    if (!container) return;
    const list = loadMilestones();
    container.innerHTML = list.map(m => {
        const cls = m.unlocked ? 'dashboard-milestone' : 'dashboard-milestone';
        const action = m.claimed ? '<button class="btn-secondary" disabled>Claimed</button>' : (m.unlocked ? `<button class="btn-primary" onclick="claimMilestone('${m.id}'); renderDashboardMilestones(); renderMilestones();">Claim</button>` : `<button class="btn-secondary" disabled>Locked</button>`);
        return `
            <div class="${cls}" data-id="${m.id}">
                <div style="flex:1;text-align:left">
                    <strong>${m.title}</strong>
                    <div style="font-size:0.9rem;color:#666">${m.description}</div>
                </div>
                <div style="margin-left:12px">${action}</div>
            </div>
        `;
    }).join('');
}
...
function showRewardModal(reward) {
    ...
    if (reward.type === 'planner') {
        ...
        a.onclick = () => window.open(new URL(reward.href || 'study_planner.html', location.href).href, '_blank');
    ...
    // hide milestone celebration if still open
    const ms = document.getElementById('milestoneModal'); if(ms) ms.classList.remove('active');
...
// (milestones button removed from UI)
const milestonesBtn = document.getElementById('milestonesBtn');
if (milestonesBtn) {
    // safety: if button exists in DOM for any reason, wire it; otherwise do nothing
